...
2025-01-28 copied and adapted from the original
https://github.com/BIT-DYN/OpenGraph/tree/master?tab=readme-ov-file
for testing in CURB-SG
...
sys.path.append("/workspaces/collaborative-scene-graphs/src/OpenGraph/Tag2Text")
...
class MyAutomaticMaskGenerator:
    ...
    # 初始化，把参数传入进来
    def __init__(
        self, tagging_model, grounding_dino_model, tap_model, sbert_model=None
    ):
        ...
        self.add_classes = self.cityscapes_classes
    ...
    @torch.no_grad()
    def generate(
        self, image: np.ndarray, save_path: Optional[str] = None, save_vis: bool = True
    ) -> Tuple[List[NDArray], List[str], List[float]]:

        if self.tagging_model is not None:
            ...
            # use fixed set of classes for every img
            classes = self.cityscapes_classes
        ...
        img_batch = im_vstack(
            img_list, fill_value=self.tap_model.pixel_mean_value, size=(1024, 1024)
        )
        ...
        captions = ["NOCAPTION"] * len(concepts)
        ...
        if save_vis:
            ...
            plt.imshow(vis_img)
            self.show_masks(masks, concepts, captions, plt.gca(), detections, scores)
            # self.show_masks(
            #     masks,
            ...
            if save_path:
                ...
                non_zero_pixels = cv2.findNonZero(
                    cv2.cvtColor(vis_img, cv2.COLOR_BGR2GRAY)
                )
    ...
    def show_masks(self, masks, concepts, captions, ax, detections, scores):
        """
        保存图像可视化用
        """
        for i, (mask, concept, caption, score) in enumerate(
            zip(masks, concepts, captions, scores)
        ):
            if concept != "traffic light" and concept != "traffic sign":
                continue
            # 找到mask中True值的坐标
            true_coords = np.argwhere(mask)
            if len(true_coords) > 0:
                ...
                color = np.concatenate(
                    [np.random.random(3), np.array([1])], axis=0
                )  # 调整颜色透明度
                ax.imshow(
                    mask.reshape(mask.shape[-2:] + (1,)) * color.reshape(1, 1, -1),
                    alpha=0.7,
                    label=f"Mask {i+1}",
                )
                # 展示box
