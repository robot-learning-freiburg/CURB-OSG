...
class MasaTaoTracker(BaseTracker):
    ...
    def set_state(self, state_id: int):
        if self.current_state == state_id:
            return
        # store old state
        self.states[self.current_state] = (self.num_tracks, self.tracks, self.backdrops)
        # try to load requested state, if it exists. else create new state
        if state_id in self.states.keys():
            self.num_tracks, self.tracks, self.backdrops = self.states[state_id]
        else:
            self.reset()
