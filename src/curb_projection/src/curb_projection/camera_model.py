...
class CameraModel:
    ...
    def project(self, xyz, image_size):
        ...
        Returns:
            ...
            list: correspondences between points and pixels. Will be a list of m indices pointing to entries in xyz.
        ...
        uv = np.vstack(
            (
                self.focal_length[0] * xyzw[0, :] / xyzw[2, :]
                + self.principal_point[0],
                self.focal_length[1] * xyzw[1, :] / xyzw[2, :]
        ...
        correspondences = np.array([in_front[i] for i in in_img])

        return uv[:, in_img], np.ravel(xyzw[2, in_img]), correspondences
    
    def get_projected_size(self, obj_height, obj_width, distance):
        """
        Calculate the size of the object in the image plane.
        
        Args:
            obj_height (float): height of the object in meters
            obj_width (float): width of the object in meters
            distance (float): distance of the object from the camera in meters
        Returns:
            tuple: (height, width) of the object in pixels
        """

        # Calculate the height and width of the object in pixels
        height = obj_height * self.focal_length[1] / distance
        width = obj_width * self.focal_length[0] / distance
    ...
    def __get_model_name(self, images_dir):
        ...
        if self.camera == "stereo":
            ...
            if self.camera_sensor == "left":
                ...
                raise RuntimeError(
                    "Unknown camera model for given directory: " + images_dir
                )
        else:
            return self.camera
