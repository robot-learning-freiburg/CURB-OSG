...
float xy2theta(const float &_x, const float &_y) {
    ...
    return -1;  // redundant, but avoids compiler warnings
}  // xy2theta
...
void SCManager::makeAndSaveScancontextAndKeys(const hdl_graph_slam::KeyFrame::Ptr keyframe) {
    Eigen::MatrixXd sc = makeScancontext(*keyframe->cloud);  // v1
...
int SCManager::find_id_mapping(int kf_id) {
    /* scan context manager is built for one agent and uses IDs starting with 0 for
     * the first frame added etc..
     * CURB SLAM uses per-agent ids, so this search is used to find the mapping
     * keyframe id -> sc manager id
     */
    for(int i = 0; i < keyframes.size(); i++) {
        if(keyframes[i]->keyframe_id == kf_id) return i;
    }
    return -1;
}

std::tuple<hdl_graph_slam::KeyFrame::Ptr, float, float> SCManager::detectLoopClosureID(const hdl_graph_slam::KeyFrame::Ptr current_kf) {
    int loop_id{-1};  // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")
    hdl_graph_slam::KeyFrame::Ptr result_kf = nullptr;

    // map the keyframe ID back to sc_manager ids
    int sc_id = find_id_mapping(current_kf->keyframe_id);
    auto curr_key = polarcontext_invkeys_mat_[sc_id];  // current observation (query)
    auto curr_desc = polarcontexts_[sc_id];            // current observation (query)

    /*
     * step 1: candidates from ringkey tree_
     */
    if((int)polarcontext_invkeys_mat_.size() < NUM_EXCLUDE_RECENT + 1) {
        std::tuple<hdl_graph_slam::KeyFrame::Ptr, float, float> result{result_kf, 0.0, 0.0};
    ...
    if(min_dist < SC_DIST_THRES) {
        loop_id = nn_idx;
        result_kf = keyframes[loop_id];

        // std::cout.precision(3);
        // cout << "[Loop found] Nearest distance: " << min_dist << " btn " << current_kf->keyframe_id << " and " << keyframes[nn_idx]->keyframe_id << "." << endl;
        // cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    } else {
        // std::cout.precision(3);
        // cout << "[Not loop] Nearest distance: " << min_dist << " btn " << current_kf->keyframe_id - 1 << " and " << keyframes[nn_idx]->keyframe_id << "." << endl;
        // cout << "[Not loop] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }

    // To do: return also nn_align (i.e., yaw diff)
    float yaw_diff_rad = deg2rad(nn_align * PC_UNIT_SECTORANGLE);
    std::tuple<hdl_graph_slam::KeyFrame::Ptr, float, float> result{result_kf, min_dist, yaw_diff_rad};
