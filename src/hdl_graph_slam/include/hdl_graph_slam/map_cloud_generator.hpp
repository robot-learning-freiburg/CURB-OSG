...
class MapCloudGenerator {
  public:
     ...
     */
    pcl::PointCloud<PointT>::Ptr generate(const std::vector<KeyFrameSnapshot::Ptr> &keyframes, double resolution, bool apply_time_shading) const;

    pcl::PointCloud<PointT>::Ptr filterPointCloud(const pcl::PointCloud<PointT>::Ptr &input_cloud, const std::vector<int> classes_for_higher_resolution, const float resolution) const;
