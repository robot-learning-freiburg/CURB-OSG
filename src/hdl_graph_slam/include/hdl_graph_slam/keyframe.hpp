...
struct KeyFrame {
  public:
    ...
    KeyFrame(const ros::Time &stamp, const int id, const Eigen::Isometry3d &odom, double accum_distance, const pcl::PointCloud<PointT>::ConstPtr &cloud);
  ...
  public:
    ...
    friend std::ostream &operator<<(std::ostream &os, const KeyFrame &kf) {
        os << "Keyframe ID: " << kf.keyframe_id;
        //            if (kf.node != nullptr) {
        //                os << "(Node ID: " << kf.node->id() << ")";
        //            }
        return os;
    }
};
