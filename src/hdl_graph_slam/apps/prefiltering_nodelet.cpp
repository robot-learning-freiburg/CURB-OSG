...
class PrefilteringNodelet : public nodelet::Nodelet {
  ...
  private:
    void initialize_params() {
        ...
        use_crop_filter = private_nh.param<bool>("use_crop_filter", true);
        ...
        pcl::CropBox<PointT>::Ptr crop_box(new pcl::CropBox<PointT>());
        crop_box->setMin(Eigen::Vector4f(-100, -100, -5, 1));
        crop_box->setMax(Eigen::Vector4f(100, 100, 100.0, 1));
        crop_filter = crop_box;
    }
    ...
    void cloud_callback(const pcl::PointCloud<PointT> &src_cloud_r) {
        ...
        if(!base_link_frame_id.empty()) {
            if(!tf_listener.canTransform(base_link_frame_id, src_cloud->header.frame_id, ros::Time(0))) {
                std::cerr << "failed to find transform between " << base_link_frame_id << " and " << src_cloud->header.frame_id << std::endl;
            }

            tf::StampedTransform transform;
            tf_listener.waitForTransform(base_link_frame_id, src_cloud->header.frame_id, ros::Time(0), ros::Duration(2.0));
        ...
        filtered = crop(filtered);

        // small objects disabled for now
        ...
        //   range_cond->addComparison(pcl::FieldComparison<PointT>::ConstPtr(new pcl::FieldComparison<PointT>("intensity", pcl::ComparisonOps::EQ, classes_for_high_resolution[i])));
        // }
        ...
        // ROS_DEBUG_STREAM("small objects: " << cloud_of_small_objects->points.size());
    ...
    pcl::PointCloud<PointT>::ConstPtr crop(const pcl::PointCloud<PointT>::ConstPtr &cloud) const {
        if(!use_crop_filter) {
            return cloud;
        }
  ...
  private:
    ...
    pcl::Filter<PointT>::Ptr crop_filter;
};
