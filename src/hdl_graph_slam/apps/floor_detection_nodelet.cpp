...
class FloorDetectionNodelet : public nodelet::Nodelet {
  public:
    ...
    virtual void onInit() {
        ...
        agent_no = private_nh.param<std::string>("agent_no", "104");
        ...
        points_sub = nh.subscribe(filtered_points_topic, 256, &FloorDetectionNodelet::cloud_callback, this);
        floor_pub = nh.advertise<hdl_graph_slam::FloorCoeffs>(floor_detection_topic + "/floor_coeffs", 32);

        read_until_pub = nh.advertise<std_msgs::Header>(floor_detection_topic + "/read_until", 32);
        floor_filtered_pub = nh.advertise<sensor_msgs::PointCloud2>(floor_detection_topic + "/floor_filtered_points", 32);
        floor_points_pub = nh.advertise<sensor_msgs::PointCloud2>(floor_detection_topic + "/floor_points", 32);
    }
  ...
  private:
    ...
    void initialize_params() {
        tilt_deg = private_nh.param<double>("tilt_deg",
                                            0.0);  // approximate sensor tilt angle [deg]
        sensor_height = private_nh.param<double>("sensor_height",
                                                 2.0);  // approximate sensor height [m]
        height_clip_range = private_nh.param<double>("height_clip_range",
                                                     1.0);  // points with heights in [sensor_height - height_clip_range, sensor_height + height_clip_range] will be used for floor detection
        floor_pts_thresh = private_nh.param<int>("floor_pts_thresh",
                                                 512);  // minimum number of support points of RANSAC to accept a detected floor plane
        floor_normal_thresh = private_nh.param<double>("floor_normal_thresh",
                                                       10.0);  // verticality check thresold for the detected floor plane [deg]
        use_normal_filtering = private_nh.param<bool>("use_normal_filtering",
                                                      true);  // if true, points with "non-"vertical normals will be filtered before RANSAC
        normal_filter_thresh = private_nh.param<double>("normal_filter_thresh",
                                                        20.0);  // "non-"verticality check threshold [deg]

        points_topic = private_nh.param<std::string>("points_topic", points_topic);
    }
