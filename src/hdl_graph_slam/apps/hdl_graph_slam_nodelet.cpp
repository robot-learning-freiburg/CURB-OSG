...
class HdlGraphSlamNodelet : public nodelet::Nodelet {
  public:
    ...
    virtual void onInit() {
        ...
        odom_frame_id = private_nh.param<std::string>("odom_frame_id", "robotcar_" + agent_no + "/odom");
        ...
        floor_detection_topic = private_nh.param<std::string>("floor_detection_topic", "/floor_detection_" + agent_no);
        skipping_obs = false;
        ...
        cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, filtered_points_topic, 32));
        sync.reset(new message_filters::Synchronizer<ApproxSyncPolicy>(ApproxSyncPolicy(32), *odom_sub, *cloud_sub));
        sync->registerCallback(boost::bind(&HdlGraphSlamNodelet::cloud_callback, this, _1, _2));

        floor_sub = nh.subscribe(floor_detection_topic + "/floor_coeffs", 1024, &HdlGraphSlamNodelet::floor_coeffs_callback, this);
        command_sub = nh.subscribe(hdl_graph_slam_topic + "/command", 1024, &HdlGraphSlamNodelet::command_callback, this);
        ...
        dump_service_server = mt_nh.advertiseService(hdl_graph_slam_topic + "/dump", &HdlGraphSlamNodelet::dump_service, this);
        save_map_service_server = mt_nh.advertiseService(hdl_graph_slam_topic + "/save_map", &HdlGraphSlamNodelet::save_map_service, this);
  ...
  private:
    ...
    void cloud_callback(const nav_msgs::OdometryConstPtr &odom_msg, const sensor_msgs::PointCloud2::ConstPtr &cloud_msg) {
        ...
        ROS_DEBUG_STREAM("size of cloud: " << cloud->points.size());
        ROS_DEBUG_STREAM("keyframe stamp: " << stamp);

        if(base_frame_id.empty()) {
            base_frame_id = cloud_msg->header.frame_id;
        }

        // check if this odom is far enough from the previous keyframe. If not, return immediately
        ...
        ROS_DEBUG_STREAM("created keyframe with ID: " << keyframe_id);
    ...
    void sendKeyframesToServer() {
        ...
        ROS_INFO("Sending keyframes - Index: %i/%li - sending: %li Keyframes", indexSendKeyframes, keyframes.size(),
                 send_keyframes.size());
        ...
        for(const auto &keyframe : send_keyframes) {
            ...
            if(indexSendKeyframes == 0) {
                ...
                if(!tf_listener.canTransform("world", "robotcar_" + agent_no + "/gt/base_link", keyframe->stamp, &error_msg)) {
                    ROS_ERROR("Initial pose not found: %s", error_msg.c_str());
                    ros::shutdown();
                }

                tf::StampedTransform initial_pose_tf;
                tf_listener.lookupTransform("world", "robotcar_" + agent_no + "/gt/base_link", keyframe->stamp, initial_pose_tf);
                tf::poseTFToEigen(initial_pose_tf, initial_pose);

                ROS_INFO_STREAM("HDL: Initial Pose received and set: trans:" << std::endl
                                                                            << initial_pose.translation() << std::endl
                                                                            << "rot=" << std::endl
                                                                            << initial_pose.rotation());
                geometry_msgs::Pose initial_pose_msg;
                tf::poseTFToMsg(initial_pose_tf, initial_pose_msg);
                double initial_shift_x, initial_shift_y;
                nh.getParam("initial_shift_x", initial_shift_x);
                nh.getParam("initial_shift_y", initial_shift_y);
                initial_pose_msg.position.x += initial_shift_x;
                initial_pose_msg.position.y += initial_shift_y;
    ...
    bool flush_keyframe_queue() {
        std::lock_guard<std::mutex> lock(keyframe_queue_mutex);
        ROS_INFO_STREAM_THROTTLE(2.0, "Keyframe queue size in hdl graph slam nodelet " << keyframe_queue.size());
    ...
    void optimization() {
        ...
        if(odom2map_pub.getNumSubscribers()) {
            geometry_msgs::TransformStamped ts = matrix2transform(keyframe->stamp, trans_odom2map, map_frame_id, odom_frame_id);
