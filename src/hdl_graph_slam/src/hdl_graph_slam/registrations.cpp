...
pcl::Registration<pcl::PointXYZINormal, pcl::PointXYZINormal>::Ptr select_registration_method(ros::NodeHandle &pnh) {
    ...
    if(registration_method == "FAST_GICP") {
        ...
        return gicp;
    } else if(registration_method == "NANO_GICP") {
        std::cout << "registration: NANO_GICP" << std::endl;
        // https://github.com/engcang/nano_gicp?tab=readme-ov-file#use-case
        nano_gicp::NanoGICP<PointT, PointT> nano_gicp;
        nano_gicp.setMaxCorrespondenceDistance(pnh.param<double>("reg_max_correspondence_distance", 2.5));
        nano_gicp.setNumThreads(pnh.param<int>("reg_num_threads", 0));
        nano_gicp.setCorrespondenceRandomness(pnh.param<int>("reg_correspondence_randomness", 20));
        nano_gicp.setMaximumIterations(pnh.param<int>("reg_maximum_iterations", 64));
        nano_gicp.setTransformationEpsilon(pnh.param<double>("reg_transformation_epsilon", 0.01));

        nano_gicp.setEuclideanFitnessEpsilon(pnh.param<double>("reg_euclidean_fitness_epsilon", 0.01));
        nano_gicp.setRANSACIterations(pnh.param<double>("reg_ransac_iterations", 5));
        nano_gicp.setRANSACOutlierRejectionThreshold(pnh.param<double>("reg_ransac_outlier_detection_thresh", 0.5));

        return pcl::make_shared<nano_gicp::NanoGICP<PointT, PointT>>(nano_gicp);
    }
#ifdef USE_VGICP_CUDA
